...
**Sync env (manual):** [`.github/workflows/sync-env-to-server.yml`](.github/workflows/sync-env-to-server.yml) pushes API + Postgres compose fragments to the VPS (same **`sync-env-to-server`** reusable workflow as hear-the-music-tree-api). Run from **Actions → Sync env to server** after changing Postgres / R2 / other API-related GitHub env. **`DATABASE_URL`** is **not** in this fragment—it is set by [infrastructure](https://github.com/BehindTheMusicTree/infrastructure) on the server (same idea as **`PORT`** / **`TMD_ADMIN_API_LISTEN_PORT`**). **`REDEPLOYMENT_ROOT`** must be the Music Deck admin tree (e.g. **`/var/webhook/redeployment-the-music-deck-admin`**); **`app_name`** for the API fragment is **`TMD_ADMIN_API_APP_NAME`**, and for Postgres **`TMD_ADMIN_API_APP_NAME` + `DB_APP_NAME_SUFFIX`** (required GitHub Variable, same as infrastructure). See [`CONTRIBUTING.md`](CONTRIBUTING.md).
