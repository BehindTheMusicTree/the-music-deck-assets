...
| Secret   | **`TMD_ADMIN_DATABASE_URL`**                                                      | Per-environment (**STAGING** / **PROD**): Nest **`DATABASE_URL`** (DB host in compose is **`${TMD_ADMIN_API_APP_NAME}${DB_APP_NAME_SUFFIX}-<env>`**, e.g. **`_db`** → `...@myapi_db-staging:5432/...`). Written to **`/tmp/sync-env-<TMD_ADMIN_API_APP_NAME>-<env>.env`**; infra redeploy promotes into **`compose/<TMD_ADMIN_API_APP_NAME>.env`**. |
| Secret   | **`TMD_ADMIN_POSTGRES_PASSWORD`**                                                 | Postgres **`POSTGRES_PASSWORD`** for the DB compose fragment: sync uploads **`/tmp/sync-env-<TMD_ADMIN_API_APP_NAME><DB_APP_NAME_SUFFIX>-<env>.env`** (same basename as **`TMD_ADMIN_DB_APP_NAME`** / infra derivation). |
| Variable | **`TMD_ADMIN_POSTGRES_USER`**, **`TMD_ADMIN_POSTGRES_DB`** _(optional)_         | Defaults **`postgres`** / **`app`** in the DB fragment when unset. |
