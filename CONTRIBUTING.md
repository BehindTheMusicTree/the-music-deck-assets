...
| Secret   | **`TMD_ADMIN_DB_POSTGRES_USER_PASSWORD`**                                          | Bootstrap superuser **`POSTGRES_PASSWORD`** (official **`postgres`** image treats **`POSTGRES_USER`** as superuser). DB fragment: **`/tmp/sync-env-<TMD_ADMIN_API_APP_NAME><DB_APP_NAME_SUFFIX>-<env>.env`**.                                                                                                                                                                                                           |
...
| Variable | **`TMD_ADMIN_DB_POSTGRES_USER`**, **`TMD_ADMIN_DB_POSTGRES_DB_NAME`** _(optional)_ | Bootstrap **`POSTGRES_USER`** / **`POSTGRES_DB`** (defaults **`postgres`** / **`app`**). The DB name is read on the server when building **`DATABASE_URL`** (must match what the app expects).                                                                                                                                                                                                                            |
