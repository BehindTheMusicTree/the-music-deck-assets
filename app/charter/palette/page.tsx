...
const tokens = [
  {
    name: "--bg",
    hex: "#09080d",
    usage: "Main background (near-black violet)",
  },
  ...
  return (
    <div className="px-6 py-10 flex flex-col items-center">
      ...
      <p className="font-garamond italic text-muted max-w-[600px] text-center mb-12">
        ...
        <code className="font-mono">:root</code> and used consistently across
        the UI.
      </p>
      ...
      <div className="w-full max-w-[700px] flex flex-col gap-2">
        {tokens.map(({ name, hex, usage }) => (
          <div
            key={name}
            className="flex items-center gap-4 bg-card border border-ui-border rounded px-4 py-3"
          >
            <div
              className="w-10 h-10 rounded-[3px] shrink-0 border border-white/[0.06]"
              style={{ background: hex }}
            />
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline gap-3 flex-wrap">
                <span className="font-mono text-gold">{name}</span>
                <span className="font-mono text-muted">{hex}</span>
              </div>
              <div className="font-garamond text-sm text-muted mt-0.5">
                {usage}
              </div>
            </div>
          </div>
