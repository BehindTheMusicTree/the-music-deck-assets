...
export default function WishlistDeckTable({
  ...
  const visibleRows = useMemo(() => {
    ...
    return [...rows].sort((a, b) => compareWishlistRows(a, b, sortKey, sortAsc));
  }, [
    filterKind,
    filterAppGenre,
    filterLineGenre,
        ...
        <table className="w-full min-w-[1280px] border-collapse text-left">
          ...
          <tbody className="font-garamond text-[15px] text-white/95">
            {visibleRows.map((entry) => {
              const { rowKey, kind, card, theme, ordinal, appGenreLabel, intensity } =
                entry;
              const prompt = card.artworkPrompt?.trim();
              const promptPrev = prompt
                ? artworkPromptPreview(prompt)
                : null;
              return (
                <tr
                  key={rowKey}
                ...
                >
                  ...
                  <td className="py-2.5 px-2 text-muted align-middle">{kind}</td>
